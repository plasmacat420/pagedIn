...
export default function DeployOptions({ html, deployToken, parsedName, firstName, onDeployed, onError }) {
  const [deploying, setDeploying] = useState(null) // null | 'pagedin' | 'self'
  const [githubToken, setGithubToken] = useState(null) // stored in memory only
  const [loadingOAuth, setLoadingOAuth] = useState(false)

  // Listen for OAuth popup returning with code
  useEffect(() => {
  ...
  const handleGithubCode = async (code) => {
    ...
    try {
      ...
      // Auto-trigger self-deploy
      await runSelfDeploy(access_token)
    } catch (err) {
      onError(err.message || 'GitHub authentication failed.')
    } finally {
  ...
  const runSelfDeploy = async (token) => {
    ...
    try {
      ...
      onError(err.message || 'Deployment to your GitHub failed.')
      setDeploying(null)
    }
  }
  ...
  const connectGitHub = () => {
    if (!GITHUB_CLIENT_ID) {
      onError('Self-deploy is not configured. Please use PagedIn hosting instead.')
      return
    }
    const redirect = `${window.location.origin}/oauth/github`
  ...
  return (
    <section className="max-w-3xl mx-auto px-4 pb-24">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-400 text-sm">Both options create a real GitHub Pages site — no expiration, no watermark.</p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">

        {/* Option 1: PagedIn hosted — RECOMMENDED */}
        <div className={`card border-2 ${deploying === 'pagedin' ? 'border-brand-500' : 'border-slate-800 hover:border-slate-600'} transition-all flex flex-col`}>
          ...
          <ul className="space-y-1.5 text-sm text-slate-400 mb-6 flex-1">
            ...
            <li className="flex items-center gap-2"><span className="text-emerald-400">✓</span> One click, instant deploy</li>
            <li className="flex items-center gap-2"><span className="text-emerald-400">✓</span> URL: pagedin-sites.github.io/your-name</li>
        ...
        <div className={`card border-2 ${deploying === 'self' ? 'border-slate-500' : 'border-slate-800 hover:border-slate-600'} transition-all flex flex-col`}>
          <div className="flex items-start justify-between mb-3">
            <div>
          ...
          <ul className="space-y-1.5 text-sm text-slate-400 mb-6 flex-1">
            <li className="flex items-center gap-2"><span className="text-slate-500">✓</span> Your own GitHub account</li>
            <li className="flex items-center gap-2"><span className="text-slate-500">✓</span> URL: yourusername.github.io/my-resume</li>
            <li className="flex items-center gap-2"><span className="text-slate-500">✓</span> Full ownership &amp; control</li>
